...
use crate::commands;
pub use crate::pipe_report::attach_change_report;
...
pub fn unknown_cmd_error(name: &str) -> crate::BoxError {
    if name.is_empty() {
...
pub async fn dispatch_single(
    ...
    let result: Result<Value, crate::BoxError> = match verb {
        None => Err(unknown_cmd_error(cmd_name)),
        ...
        // A batch inside a batch. Refused by name, not as an unknown word: the caller's
        // recovery for "unknown command" is to check the spelling, and the recovery here is
        // to hoist the commands into the outer batch. Reachable, and it used to answer
        // `Unknown command: batch`, which sends the reader looking for a typo.
        PipeVerb::Batch => Err("batch cannot be nested inside a batch; hoist its commands into the outer batch".into()),
    },
    };
